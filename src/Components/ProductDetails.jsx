...
export default function ProductDetails() {
  ...
  if (!id) return <h2 className="text-center">Invalid Product ID</h2>;
  if (isLoading || add || favo) return <Loading />;
  if (isError) return <h2 className="text-center">{error.message}</h2>;
  if (!data) return <h2 className="text-center">No product data available</h2>;

  console.log("Product Data:", data);

  const handleWishList = () => {
    wish(data._id, {
      onSuccess: () => setHeart(!heart),
      onError: (err) => console.error("Wishlist error:", err),
    });
  };
  ...
  return (
    <div className="container">
      <Helmet>
        <title>{data?.title ? data.title.substring(0, 60) : "Product Details"}</title>
        <meta
          name="description"
          content={data?.description ? data.description.substring(0, 160) : "View product details on our e-commerce platform"}
        />
      </Helmet>
      <div className="row align-items-center my-3">
            ...
            {data?.images?.length ? (
              data.images.map((img, index) => (
                <img
                  ...
                  className="w-100"
                  src={img}
                  alt={data?.title ? `${data.title} ${index + 1}` : "Product image"}
        ...
        <div className="col-12 col-md-8">
          <div className="d-flex justify-content-between">
            <h3 className="fw-bold">{data?.title}</h3>
            <i
              className={!heart ? "fa-solid fa-heart-pulse heart text-main" : "fa-solid fa-heart heart text-main"}
          ...
          <button
            className="btn btn-bord form-control my-3 fw-bold"
